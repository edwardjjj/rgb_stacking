...
from dm_robotics.moma import action_spaces, subtask_env, subtask_env_builder
from dm_robotics.moma.utils import mujoco_collisions

from rgb_stacking import reward_functions, stack_rewards, task
...
class ObservationSet(int, enum.Enum):
    ...
    INTERACTIVE_IMITATION_LEARNING = (3, _INTERACTIVE_IMITATION_LEARNING_OBSERVATIONS)
...
def rgb_stacking(
    ...
    # base task.
    effectors_action_spec = rgb_task.effectors_action_spec(physics=task_env.physics)
    robot_action_spaces = []
    ...
    composite_action_space = af.CompositeActionSpace(robot_action_spaces)
    ...
    # env_builder.add_preprocessor(
    #     observation_transforms.MergeObservations(
    ...
    env_builder.add_preprocessor(
        observation_transforms.MergeObservations(
            obs_to_merge=[
                "robot0_arm_joint_previous_action",
    ...
    # Rename observations.
    env_builder.add_preprocessor(
        observation_transforms.RenameObservations(obs_mapping, raise_on_missing=False)
    )
    ...
    env_builder.add_preprocessor(
        observation_transforms.StackObservations(
            obs_to_stack=list(set(_STATE_OBSERVATIONS) - action_obs - single_value_obs),
    ...
    env_builder.add_preprocessor(subtask_termination.MaxStepsTermination(_MAX_STEPS))
